...
$students = mysqli_query($conn, "SELECT * FROM poll ORDER BY id DESC");
?>
        ...
        <table class="table is-striped is-narrow is-hoverable is-fullwidth" id="id01">
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($students)) {
                    $poll_id = $row['id'];
                    $options = mysqli_query($conn, "SELECT * FROM options WHERE poll_id='$poll_id'")
                ?>
                    <tr class="item">
                        <td><?php echo $row['id'] ?></td>
                        <td><?php echo $row['poll_by']; ?></td>
                        <td><?php echo $row['user_id']; ?></td>
                        <td><?php echo $row['college_id'] ?></td>
                        <td><?php echo $row['question'] ?></td>
                        <td><?php echo mysqli_num_rows($options); ?></td>
